def build_classification_prompt(message: str) -> str:
    ...
    prompt = (
        ...
        "- faq (pertanyaan umum tentang hotel seperti fasilitas, kolam renang, lokasi, parkir, wifi)\n"
        "- greeting (sapaan santai awal atau ungkapan pembuka ingin bertanya seperti halo, hi, good morning, permisi, saya ingin tanya dulu)\n"
        ...
        "- complaint (keluhan mengenai fasilitas rusak/kotor/bermasalah seperti wifi tidak connect, AC mati, kamar kotor)\n"
        "- cancellation (permintaan pembatalan pemesanan)\n"
        "- refund (permintaan pengembalian dana)\n"
        "- refund_inquiry (pertanyaan mengenai proses atau durasi pengembalian dana)\n"
        "- reschedule (permintaan mengubah tanggal atau jadwal booking yang sudah ada)\n"
        "- amenities (permintaan barang tambahan seperti handuk, air mineral, selimut, sabun)\n"
        "- escalate (permintaan berbicara langsung dengan staf, CS, atau manusia)\n"
        ...
        "- \"wifi tidak connect\" -> complaint\n"
        "- \"kamar mandi kotor\" -> complaint\n"
        "- \"AC kamar saya rusak\" -> complaint\n"
        "- \"minta handuk tambahan\" -> amenities\n"
        "- \"tolong kirim air mineral\" -> amenities\n"
        "- \"bagaimana proses refund?\" -> refund_inquiry\n"
        "- \"uang saya kapan kembali?\" -> refund\n"
        "- \"mau connect ke customer service\" -> escalate\n"
        "- \"bicara dengan staff\" -> escalate\n"
        ...
        "- \"Mau ubah tanggal booking saya.\" -> reschedule\n"
        "- \"Halo, selamat pagi.\" -> greeting\n"
        "- \"xyzabc123 tidak jelas\" -> unknown\n\n"
...
def build_multiturn_prompt(history: list[dict], message: str) -> str:
    ...
    prompt = (
        ...
        "- greeting (sapaan santai / ungkapan ingin bertanya)\n"
